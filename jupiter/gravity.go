...
// function AverageSpeed takes in a slice of Universe objects and returns the average speed of each bodies in the universe
func AverageSpeed(u []Universe) []float64 {
	numBody := len(u[0].bodies)
	speedList := make([]float64, numBody)

	//range through all the different timepoints of the universe and acquire the specific speed of each body at the specific timepoint.
	for timeindex := range u {
		for i := range u[timeindex].bodies {
			velox := u[timeindex].bodies[i].velocity.x
			veloy := u[timeindex].bodies[i].velocity.y
			velo := math.Sqrt(velox*velox + veloy*veloy)

			//add each speed at different times to the speedlist and get a sum of speed for each body
			speedList[i] += velo
		}
	}

	//divide the sum of speeds by the timepoint number to get the average speed of each body
	for i := range speedList {
		speedList[i] = speedList[i] / float64(len(u))
	}
